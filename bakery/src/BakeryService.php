...
class BakeryService {
  ...
  public function authenticateFinalize(&$edit) {
    ...
    \Drupal::logger('user')->notice('Session opened for %name.', ['%name' => $user->getUsername()]);
   ...
   */
  public function userExternalLogin($account, $edit = []) {
    
    // Check if user is blocked or denied by access rules.
    if (user_is_blocked($account->getUsername())) {
      return FALSE;
    }
    ...
    if ($cookie) {
      ...
      if ($user->id() == 0) {
        ...
        if (!$account && $this->config->get('bakery_is_master') == 0 && $cookie['master'] == 0) {
          $count = $this->db->select('users_field_data', 'u')->fields('u', ['uid'])
            ->condition('init', $cookie['init'])
            ->countQuery()->execute()->fetchField();
          if ($count > 1) {
            // Uh oh.
            \Drupal::logger('bakery')->notice('Account uniqueness problem: Multiple users found with init %init.', ['%init' => $cookie['init']]);
            \Drupal::messenger()->addMessage(t('Account uniqueness problem detected. <a href="@contact">Please contact the site administrator.</a>', ['@contact' => $this->config->get('bakery_master') . 'contact']), 'error');
          }
          if ($count == 1) {
            $account = \Drupal::entityManager()->getStorage('user')->loadByProperties(['init' => $cookie['init']]);
            if (is_array($account)) {
              $account = reset($account);
            }
            if ($account) {
        ...
        if (!$account && $this->config->get('bakery_is_master') == 0 && $cookie['master'] == 0) {
          $checks = TRUE;
          $mail_count = $this->db->select('users_field_data', 'u')->fields('u', ['uid'])
            ->condition('uid', $user->id(), '!=')
            ->condition('mail', '', '!=')
            ->where('LOWER(mail) = LOWER(:mail)', [':mail' => $cookie['mail']])
            ->countQuery()->execute()->fetchField();
          if ($mail_count > 0) {
            $checks = FALSE;
          }
          $name_count = $this->db->select('users_field_data', 'u')->fields('u', ['uid'])
            ->condition('uid', $user->id(), '!=')
            ->where('LOWER(name) = LOWER(:name)', [':name' => $cookie['name']])
            ->countQuery()->execute()->fetchField();
          if ($name_count > 0) {
            $checks = FALSE;
          }
          $init_count = $this->db->select('users_field_data', 'u')->fields('u', ['uid'])
            ->condition('uid', $user->id(), '!=')
            ->condition('init', $cookie['init'], '=')
            ->where('LOWER(name) = LOWER(:name)', [':name' => $cookie['name']])
            ->countQuery()->execute()->fetchField();
          ...
          if ($checks) {
            ...
            \Drupal::messenger()->addMessage(t('Your user account on %site appears to have problems. Would you like to try to <a href="@url">repair it yourself</a>?', ['%site' => \Drupal::config('system.site')->get('name'), '@url' => \Drupal::url('bakery.repair')]));

            \Drupal::messenger()->addMessage(Xss::filter($this->config->get('bakery_help_text')));
            $session = \Drupal::request()->getSession();
            $session->set('BAKERY_CRUMBLED', TRUE);
          }
        ...
        if ($account && $user->id() == 0) {
          ...
          if ($login) {
            ...
            $data = $this->validateCookie('OATMEAL');
            $destination = isset($data['data']['destination']) ? $data['data']['destination'] : null;
            if ($destination && !UrlHelper::isExternal($destination)) {
              $redirect_url = Url::fromUserInput($destination)->setAbsolute()->toString();
            } else {
  ...
  public function requestAccount($name, $or_email = FALSE) {
    ...
    $master = rtrim($this->config->get('bakery_master'), '/') . '/';
    $key = $this->config->get('bakery_key');
    ...
    $account->save();
    if (!$account) {
      \Drupal::logger('bakery')->error('Unable to create stub account for @name', ['@name' => $name]);
      return FALSE;
    }
    $stub_uid = $account->id();
    ...
    if ($response->getStatusCode() != 200) {
      ...
      \Drupal::logger('bakery')->error('Received response !code from master with message @message', ['!code' => $result->code, '@message' => $message]);
      $account->delete();
      return FALSE;
    }
    ...
    foreach ($this->config->get('bakery_supported_fields') as $type => $enabled) {
      if ($enabled && isset($cookie[$type])) {
        if ($account->hasField($type)) {
          $account->{$type}->setValue($cookie[$type]);
        }
      }
    }
    // Save user.
    ...
    if ($account) {
      \Drupal::logger('bakery')->notice('Created account for @name', ['@name' => $name]);
      return $account->id();
    }

    \Drupal::logger('bakery')->error('Unable to create account for @name', ['@name' => $name]);
    $account->delete();
    return FALSE;
  }
  ...
  private function bakeryEncrypt($text) {
    ...
    $key = substr($key, 0, phpseclib_mcrypt_enc_get_key_size($td));
