...
class BakeryUncrumbleForm extends FormBase implements ContainerInjectionInterface {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $query = \Drupal::database()->select('users_field_data', 'u')
      ...
      ->condition('u.uid', 0, '!=')
      ->condition('u.mail', '', '!=')
      ->where("LOWER(u.mail) = LOWER(:mail)", [':mail' => $cookie['mail']]);
    ...
    $query = \Drupal::database()->select('users_field_data', 'u')
      ->fields('u', ['uid', 'name', 'mail'])
      ->condition('u.uid', 0, '!=')
      ->where("LOWER(u.name) = LOWER(:name)", [':name' => $cookie['name']]);
    $result = $query->execute();
    ...
    $count = \Drupal::database()->select('users_field_data', 'u')->fields('u', ['uid'])->condition('init', $cookie['init'], '=')
      ->countQuery()->execute()->fetchField();
    if ($count > 1) {
      \Drupal::messenger()->addMessage(t('Multiple accounts are associated with your master account. This must be fixed manually. <a href="@contact">Please contact the site administrator.</a>', ['%email' => $cookie['mail'], '@contact' => \Drupal::config('bakery.settings')->get('bakery_master') . 'contact']));
      $form['pass']['#disabled'] = TRUE;
      $form['submit']['#disabled'] = TRUE;
    }
    elseif ($samename && $samemail && $samename->uid != $samemail->uid) {
      \Drupal::messenger()->addMessage(t('Both an account with matching name and an account with matching email address exist, but they are different accounts. This must be fixed manually. <a href="@contact">Please contact the site administrator.</a>', ['%email' => $cookie['mail'], '@contact' => \Drupal::config('bakery.settings')->get('bakery_master') . 'contact']));
      $form['pass']['#disabled'] = TRUE;
      $form['submit']['#disabled'] = TRUE;
    }
    elseif ($samename) {
      $help = t("An account with a matching username was found. Repairing it will reset the email address to match your master account. If this is the correct account, please enter your %site password.", ['%site' => $site_name]);
      // This is a borderline information leak.
      // $form['mail']['#value'] = $samename->mail;.
      $form['mail']['#value'] = t('<em>*hidden*</em>');
      $form['mail']['#description'] = t('Will change to %new.', ['%new' => $cookie['mail']]);
    }
    elseif ($samemail) {
      $help = t("An account with a matching email address was found. Repairing it will reset the username to match your master account. If this is the correct account, please enter your %site password.", ['%site' => $site_name]);
      $form['name']['#value'] = $samemail->name;
      $form['name']['#description'] = t('Will change to %new.', ['%new' => $cookie['name']]);
    }

    $form['help'] = ['#weight' => -10, '#markup' => $help];
  ...
  public function validateForm(array &$form, FormStateInterface $form_state) {
    ...
    if (!($account && $account->id()) || $this->passwordHasher->check($form_state->getValue('pass'), $account->getPassword())) {
       ...
       */
      $form_state->setErrorByName('pass', t('Sorry, unrecognized password. If you have forgotten your %site password, please <a href="@contact">contact the site administrator.</a>', ['%site' => \Drupal::config('system.site')->get('name'), '@contact' => \Drupal::config('bakery.settings')->get('bakery_master') . ' contact']));
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $cookie = $this->bakeryService->validateCookie();
    \Drupal::database()->update('users_field_data')->fields(['init' => $cookie['init']])
      ->condition('uid', $account->id(), '=')
      ->execute();
    \Drupal::logger('bakery')->notice('uncrumble changed init field for uid %uid from %oldinit to %newinit', [
    ...
    \Drupal::messenger()->addMessage(t('Your account has been repaired.'));
